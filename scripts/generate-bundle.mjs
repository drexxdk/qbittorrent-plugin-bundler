...
function extractDownloadUrl(cell) {
  const links = extractExternalLinks(cell)
  return (
    links.find(({ url }) => /\.py([?#].*)?$/i.test(url) || /raw|githubusercontent|Scare\.ca/i.test(url))
      ?.url ?? links[0]?.url
  )
}
...
async function downloadPlugin(plugin, zip, usedNames) {
  const response = await fetch(plugin.downloadUrl, { headers: requestHeaders, redirect: 'follow' })
  if (!response.ok) {
  ...
  const fileName = pickFileName({ ...plugin, downloadUrl: finalUrl }, usedNames)
...
async function main() {
  const wikiSource = await fetchText(wikiRawUrl)
  const zip = new JSZip()
  const usedNames = new Set()

  const discoveredPlugins = sections.flatMap(({ title, slug }) => parseRows(getSectionTable(wikiSource, title), slug))
  ...
  downloadedPlugins.sort((left, right) => left.engineName.localeCompare(right.engineName))
  ...
  console.log(`Generated ${downloadedPlugins.length} plugins into ${outputZipPath}`)
